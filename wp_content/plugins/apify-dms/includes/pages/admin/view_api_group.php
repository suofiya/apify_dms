...
$display_fields = apify_get_api_group_display_fields();
$filters = apify_generate_filters_from_params(array('group_name', 'visiable', 'status'), $_REQUEST);
$filter_result = apify_get_all_api_group_list_by_filters( $filters, $paged, $page_size );

$total_paged = intval($filter_result['total'] / $page_size) + 1;
?>
<h1><?php echo __('View All Api Group Page'); ?></h1>
<div id="wpwrap">
	<div id="wpbody-content">
	<div id="icon-edit" class="icon32 icon32-posts-post"><br />
</div>
<h2><?php echo __('Api Group'); ?> <a href="admin.php?page=add-api-group" class="add-new-h2"><?php echo __('Add New'); ?></a></h2>
...
<form id="posts-filter" name="posts_filter" action="" method="post">
	<div class="tablenav top">
		<div class="alignleft actions">
			<label for="group_name"><?php echo __('name'); ?>: </label><input type="text" size="" name="group_name" value="" />
			...
			<input type="submit" name="api-group-query-submit" id="api-group-query-submit" class="button-secondary" value="Filter">
		</div>
		<br class="clear">
	</div>	
	<table class="wp-list-table widefat fixed posts" cellspacing="0">
		...
		<tbody id="the-list">
			...
			?>
			<?php foreach( $filter_result['list'] as $key=>$apiGroupObj ) { // ?>
			<tr id="api-group-<?php echo $apiGroupObj->group_id; ?>" class="status-publish format-standard hentry category-uncategorized alternate iedit author-self" valign="top">
				<th scope="row" class="check-column"><input type="checkbox" name="group_id[]" value="<?php echo $apiGroupObj->group_id; ?>"></th>
				<td class="column-id"><?php echo $apiGroupObj->group_id; ?></td>
				<td class="column-title"><span><?php echo $apiGroupObj->group_name; ?></span>
					<div class="row-actions">
					<?php if( $_REQUEST['page']=='api-group-trash' ) { ?> 
						<span class="del"><a href="admin.php?page=api-group-trash&action=reback&group_id=<?php echo $apiGroupObj->group_id; ?>" title="Del">Return</a> </span>
					<?php } else { ?>
						<span class="edit"><a href="admin.php?page=add-api-group&group_id=<?php echo $apiGroupObj->group_id; ?>" title="Edit">Edit</a> | </span> <span class="del"><a href="admin.php?page=api-group-trash&action=remove&group_id=<?php echo $apiGroupObj->group_id; ?>" title="Del">Remove</a> </span>
					<?php }	?>
					</div>
				</td>
				<td class=""><?php echo $apiGroupObj->group_title_en; ?></td>
				<td class=""><?php echo $apiGroupObj->group_title_zh; ?></td>
				<td class=""><?php echo ($apiGroupObj->visiable) ? __('Yes') : __('No'); ?></td>
				<td class=""><?php echo ($apiGroupObj->status) ? __('Yes') : __('No'); ?></td>
				<td class=""><?php echo $apiGroupObj->date_added; ?></td>
				<td class=""><?php echo $apiGroupObj->last_modified; ?></td>
			</tr>
			<?php } // end foreach?>
		</tbody>
	</table>
	...
	<div class="tablenav bottom">
		<div class="alignleft actions">
			<select name="action2">
				...
				<option value="trash"><?php echo ($_REQUEST['page'] == 'api-group-trash') ? __('Return to List') : __('Move to Trash'); ?></option>
			</select> 
