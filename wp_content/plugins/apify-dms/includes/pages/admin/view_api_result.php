...
$display_fields = apify_get_api_result_display_fields();
$filters = apify_generate_filters_from_params(array('result_name', 'visiable', 'status'), $_REQUEST);
$filter_result = apify_get_all_api_result_list_by_filters( $filters, $paged, $page_size );

$total_paged = intval($filter_result['total'] / $page_size) + 1;
?>
<h1><?php echo __('View All Api Result Page'); ?></h1>
<div id="wpwrap">
	<div id="wpbody-content">
	<div id="icon-edit" class="icon32 icon32-posts-post"><br />
</div>
<h2><?php echo __('Api Result'); ?> <a href="admin.php?page=add-api-result" class="add-new-h2"><?php echo __('Add New'); ?></a></h2>
...
<form id="posts-filter" name="posts_filter" action="" method="post">
	<div class="tablenav top">
		<div class="alignleft actions">
			<label for="result_name"><?php echo __('name'); ?>: </label><input type="text" size="" name="result_name" value="" />
			...
			<input type="submit" name="api-result-query-submit" id="api-result-query-submit" class="button-secondary" value="Filter">
		</div>
		<br class="clear">
	</div>	
	<table class="wp-list-table widefat fixed posts" cellspacing="0">
		...
		<tbody id="the-list">
			...
			?>
			<?php foreach( $filter_result['list'] as $key=>$apiResultObj ) { // ?>
			<tr id="api-result-<?php echo $apiResultObj->result_id; ?>" class="status-publish format-standard hentry category-uncategorized alternate iedit author-self" valign="top">
				<th scope="row" class="check-column"><input type="checkbox" name="result_id[]" value="<?php echo $apiResultObj->result_id; ?>"></th>
				<td class="column-id"><?php echo $apiResultObj->result_id; ?></td>
				<td class="column-title"><span><?php echo $apiResultObj->result_name; ?></span>
					<div class="row-actions">
					<?php if( $_REQUEST['page']=='api-result-trash' ) { ?> 
						<span class="del"><a href="admin.php?page=api-result-trash&action=reback&result_id=<?php echo $apiResultObj->result_id; ?>" title="Del">Return</a> </span>
					<?php } else { ?>
						<span class="edit"><a href="admin.php?page=add-api-result&result_id=<?php echo $apiResultObj->result_id; ?>" title="Edit">Edit</a> | </span> <span class="del"><a href="admin.php?page=api-result-trash&action=remove&result_id=<?php echo $apiResultObj->result_id; ?>" title="Del">Remove</a> </span>
					<?php }	?>
					</div>
				</td>	
				<td class="">
					...
					if( in_array( $api_data_type_type, array(9, 91) ) ) { //1 basic, 9 model, 91 model list
						...
						echo $api_data_type_selection_array[$apiResultObj->result_data_type_id]; 
					}
					?>
				</td>
				<td class=""><?php echo $apiResultObj->result_desc_en; ?></td>
				<td class=""><?php echo $apiResultObj->result_desc_zh; ?></td>
				<td class=""><?php echo $api_item_selection_array[$apiResultObj->item_id]; ?></td>
				<td class=""><?php echo ($apiResultObj->visiable) ? __('Yes') : __('No'); ?></td>
				<td class=""><?php echo ($apiResultObj->status) ? __('Yes') : __('No'); ?></td>
				<td class=""><?php echo $apiResultObj->date_added; ?></td>
				<td class=""><?php echo $apiResultObj->last_modified; ?></td>
			</tr>
			<?php } // end foreach?>
		</tbody>
	</table>
	...
	<div class="tablenav bottom">
		<div class="alignleft actions">
			<select name="action2">
				...
				<option value="trash"><?php echo ($_REQUEST['page'] == 'api-result-trash') ? __('Return to List') : __('Move to Trash'); ?></option>
			</select> 
		...
		<div class="tablenav-pages">
			<span class="displaying-num"><?php echo $filter_result['total']; ?> results</span>
