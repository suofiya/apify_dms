...
/**
 * 根据filters筛选所有sys parameter列表
 */
function apify_get_all_sys_parameter_list_by_filters( $filters=array(), $page_no=1, $page_size=20) {
	global $wpdb;
	
	$result = array();
	
	$cond = ' FROM '. TABLE_APIFY_API_PARAMETER;
	$cond .= ' WHERE parameter_level=1 ';
	//filter
	if( !empty($filters) ) {
...
/**
 * 通过filters获取单个sys parameter信息
 */
function apify_get_sys_parameter_by_filters($filters) {
	global $wpdb;
 ...
 * 编辑app parameter
 */
function apify_update_sys_parameter($data, $parameter_id) {
	global $wpdb;
 ...
 * 新建sys parameter
 */
function apify_insert_sys_parameter($data) {
	global $wpdb;
 ...
 * 获取所有的系统级参数列表
 */
function apifyDMSGetSysParameterList( $filters , $ids=array()) {
	global $wpdb;
